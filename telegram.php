...
<body class="bg-gray-50 dark:bg-dark-900 text-gray-800 dark:text-gray-200 pb-24 transition-colors duration-300 min-h-screen relative">
    ...
    <div id="tg-auth-container" class="min-h-screen flex flex-col items-center justify-center p-6 transition-opacity duration-500 relative overflow-hidden z-10">
        ...
        <div id="login-form-wrapper" class="hidden w-full max-w-[340px] flex-col items-center z-10 animate-fade-in-up">
            <div class="glass-panel p-8 rounded-[2.5rem] shadow-2xl w-full flex flex-col items-center relative overflow-hidden group">
                ...
                <h1 class="text-2xl font-extrabold mb-2 text-center text-gray-900 dark:text-white tracking-tight">Opening Mini App</h1>
                <p class="text-center text-gray-500 dark:text-gray-400 text-xs mb-8 leading-relaxed">
                    Connecting your Telegram account automatically. Wallet linking is available later in the Wallet section.
                </p>
                <form id="tg-login-form" class="w-full space-y-5">
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
                            <i class="fas fa-envelope"></i>
                        </div>
                        <input type="email" id="email" placeholder="FaucetPay email" class="tg-input w-full pl-11 pr-4 py-4 rounded-2xl font-semibold outline-none text-sm">
                    </div>
                    <button type="submit" id="btn-login" class="tg-button w-full py-4 rounded-2xl font-bold text-base flex justify-center items-center gap-2 shadow-lg shadow-brand-primary/20">
                        <span>Continue</span>
                        <i class="fas fa-arrow-right text-sm"></i>
                    </button>
                </form>
                <div id="login-error" class="hidden mt-5 p-4 bg-rose-500/10 text-rose-600 dark:text-rose-400 rounded-2xl text-xs font-bold w-full text-center border border-rose-500/20"></div>
                <div id="login-success" class="hidden mt-5 p-4 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded-2xl text-sm font-bold w-full text-center flex-col items-center gap-2 border border-emerald-500/20">
    ...
    <script>
        ...
        document.getElementById('tg-login-form').addEventListener('submit', function(e) {
            ...
            formData.append('telegram_id', persistentTgId);
            formData.append('telegram_username', persistentTgUsername);
            formData.append('auto_login', email ? '0' : '1');
            ...
            .then(data => {
                if (data.status === 'success') {
                    document.getElementById('tg-login-form').classList.add('hidden');
                    document.querySelector('#login-form-wrapper h1').classList.add('hidden');
                    document.querySelector('#login-form-wrapper p').classList.add('hidden');
            ...
            .catch(err => {
                ...
                btn.disabled = false;
                btn.innerHTML = '<span>Continue</span> <i class="fas fa-arrow-right text-sm"></i>';
