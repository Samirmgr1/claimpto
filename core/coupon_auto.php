...
if (!function_exists('couponCreateAutoCoupon')) {
    function couponCreateAutoCoupon($force = false) {
        ...
        if (!isset($telegramResponse['ok']) || !$telegramResponse['ok']) {
            $pdo->prepare("DELETE FROM addon_coupons WHERE id = ?")->execute([$couponId]);
            return [
                'success' => false,
                'message' => 'Coupon created but Telegram sending failed, so it was removed: ' . ($telegramResponse['description'] ?? 'Unknown Telegram error')
            ];
        }
        ...
        return [
            'success' => true,
            'message' => 'Auto coupon created and sent to Telegram.',
            'code' => $code,
            'reward' => $reward,
            'max_uses' => $maxUses,
