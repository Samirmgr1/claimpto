...
// Build random coupon ad from admin settings
$coupon_ad = null;
...
foreach ($coupon_ad_networks as $net) {
    ...
    $enabled_coupon_ads[] = $ad;
}
if (!empty($enabled_coupon_ads)) {
    shuffle($enabled_coupon_ads);
    $coupon_ad = $enabled_coupon_ads[0];
}
?>
...
<script>
    (function() {
        ...
        // Dynamic ad SDK loader for coupon page
        <?php if ($coupon_ad): ?>
        var couponAdType = '<?php echo $coupon_ad['type']; ?>';
        <?php if ($coupon_ad['type'] === 'monetag'): ?>
        var couponAdZone = '<?php echo htmlspecialchars($coupon_ad['zone_id']); ?>';
        var couponAdScript = '<?php echo htmlspecialchars($coupon_ad['script_url']); ?>';
        (function() {
            if (!document.getElementById('monetag-coupon-sdk')) {
                var s = document.createElement('script');
                s.id = 'monetag-coupon-sdk';
                s.src = couponAdScript;
                s.setAttribute('data-zone', couponAdZone);
                s.setAttribute('data-sdk', 'show_' + couponAdZone);
                document.head.appendChild(s);
            }
        })();
        <?php elseif ($coupon_ad['type'] === 'adsgram'): ?>
        var couponAdBlockId = '<?php echo htmlspecialchars($coupon_ad['block_id']); ?>';
        (function() {
        ...
        <?php elseif ($coupon_ad['type'] === 'adexora'): ?>
        var couponAdAppId = '<?php echo htmlspecialchars($coupon_ad['app_id']); ?>';
        (function() {
            if (!document.getElementById('adexora-sdk')) {
                var s = document.createElement('script');
                s.id = 'adexora-sdk';
                s.src = 'https://adexora.com/cdn/ads.js?id=' + couponAdAppId;
                document.head.appendChild(s);
            }
        })();
        <?php elseif ($coupon_ad['type'] === 'gigapub'): ?>
        var couponAdProjectId = '<?php echo htmlspecialchars($coupon_ad['project_id']); ?>';
        (function() {
            if (!document.getElementById('gigapub-sdk')) {
                var s = document.createElement('script');
                s.id = 'gigapub-sdk';
                s.src = 'https://ad.gigapub.tech/script?id=' + couponAdProjectId;
                document.head.appendChild(s);
            }
        })();
        <?php endif; ?>
        ...
        function showCouponAd(onComplete) {
            <?php if (!$coupon_ad): ?>
            onComplete();
            return;
            <?php endif; ?>

            <?php if ($coupon_ad && $coupon_ad['type'] === 'monetag'): ?>
            var showFn = 'show_' + couponAdZone;
            if (typeof window[showFn] === 'function') {
                window[showFn]().then(function() { onComplete(); }).catch(function() { onComplete(); });
            } else {
                setTimeout(function() {
                    if (typeof window[showFn] === 'function') {
                        window[showFn]().then(function() { onComplete(); }).catch(function() { onComplete(); });
            ...
            <?php elseif ($coupon_ad && $coupon_ad['type'] === 'adsgram'): ?>
            function tryAdsgram() {
                if (typeof window.Adsgram !== 'undefined') {
                    var ctrl = window.Adsgram.init({ blockId: couponAdBlockId });
                    ctrl.show().then(function(r) { onComplete(); }).catch(function() { onComplete(); });
                } else { onComplete(); }
            }
            if (typeof window.Adsgram !== 'undefined') { tryAdsgram(); }
            else { setTimeout(tryAdsgram, 2000); }
            <?php elseif ($coupon_ad && $coupon_ad['type'] === 'adexora'): ?>
            function tryAdexora() {
                if (typeof window.showAdexora === 'function') {
                    window.showAdexora().then(function() { onComplete(); }).catch(function() { onComplete(); });
                } else { onComplete(); }
            }
            if (typeof window.showAdexora === 'function') { tryAdexora(); }
            else { setTimeout(tryAdexora, 2000); }
            <?php elseif ($coupon_ad && $coupon_ad['type'] === 'gigapub'): ?>
            function tryGigapub() {
                if (typeof window.showGiga === 'function') {
                    window.showGiga().then(function() { onComplete(); }).catch(function() { onComplete(); });
                } else { onComplete(); }
            }
            if (typeof window.showGiga === 'function') { tryGigapub(); }
            else { setTimeout(tryGigapub, 2000); }
            <?php endif; ?>
        }
        ...
        if (claimBtn) {
            claimBtn.addEventListener('click', () => {
                ...
                // Show Ad before redemption (dynamic based on admin settings)
                showCouponAd(function() { executeRedemption(code); });
            });
        }
