...
$installedAddons = [];
try {
    $checkTable = $pdo->query("SHOW TABLES LIKE 'installed_addons'")->rowCount();
    if ($checkTable > 0) {
        $installedAddons = $pdo->query("SELECT * FROM installed_addons ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (Exception $e) {}
