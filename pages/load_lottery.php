...
$lottery_base_prize = (float)(getSetting('lottery_prize') ?: 500);
...
// Prize pool from tickets
$currentPrizePool = (float)($currentDraw['prize_pool'] ?? 0);
$lottery_prize = $lottery_base_prize + $currentPrizePool;
...
if (isset($_GET['claim_ticket'])) {
    ...
    try {
        ...
        // Return updated total prize for instant UI update
        $newPoolStmt = $pdo->prepare("SELECT prize_pool FROM lottery_draws WHERE id = ?");
        $newPoolStmt->execute([$drawId]);
        $newPool = (float)$newPoolStmt->fetchColumn();
        $newTotalPrize = $lottery_base_prize + $newPool;

        echo json_encode(['status' => 'success', 'ticket' => $ticketNumber, 'msg' => 'Ticket #' . $ticketNumber . ' claimed!', 'total_prize' => $newTotalPrize, 'prize_pool' => $newPool]);
    } catch (Exception $e) {
...
<div class="space-y-4 pb-4">
        ...
        <div class="relative overflow-hidden rounded-[calc(0.75rem-1px)] bg-gradient-to-br from-violet-600 via-purple-600 to-cyan-500 p-5">
            ...
            <div class="relative flex items-center gap-4">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-violet-100 text-xs font-medium mt-0.5">Win <b id="hero-prize"><?php echo number_format($lottery_prize, 0); ?></b> coins every week!</p>
                </div>
                <?php if ($drawIsActive): ?>
    ...
    <div class="grid grid-cols-3 gap-3">
        <div class="bg-white/80 dark:bg-white/5 border border-gray-200 dark:border-violet-500/10 rounded-xl p-3 text-center backdrop-blur-sm">
            <div id="stat-prize" class="text-lg font-extrabold bg-gradient-to-r from-violet-500 to-purple-500 bg-clip-text text-transparent"><?php echo number_format($lottery_prize, 0); ?></div>
    ...
    <div class="bg-white/80 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-5 shadow-sm">
        ...
        <div class="space-y-2">
            ...
            <div class="flex items-center justify-between p-3 bg-amber-50/50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-500/20 rounded-xl">
                ...
                <div class="text-xs font-extrabold text-amber-600 dark:text-amber-400">+<?php echo number_format((float)(getSetting('lottery_prize') ?: 500), 0); ?></div>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
...
window.claimTicket = function() {
    ...
    .then(function(data) {
        if (data.status === 'success') {
            ...
            // Instantly update prize displays
            if (data.total_prize !== undefined) {
                var formatted = Number(data.total_prize).toLocaleString('en-US', {maximumFractionDigits: 0});
                var heroPrize = document.getElementById('hero-prize');
                var statPrize = document.getElementById('stat-prize');
                if (heroPrize) heroPrize.textContent = formatted;
                if (statPrize) statPrize.textContent = formatted;
            }
            setTimeout(function() {
