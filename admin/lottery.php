...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_action'])) {
    ...
    if ($action === 'create_new_draw') {
        try {
            ...
            // Cancel any existing active draw for this week first
            $pdo->prepare("UPDATE lottery_draws SET status = 'cancelled' WHERE week_start = ? AND status = 'active'")->execute([$weekStart]);

            // Delete old entry for this week so we can insert fresh (unique key on week_start)
            $existing = $pdo->prepare("SELECT id FROM lottery_draws WHERE week_start = ?");
            $existing->execute([$weekStart]);
            if ($existing->fetchColumn()) {
                // Week already has a drawn/cancelled entry — use a unique key by appending draw count
                $countStmt = $pdo->query("SELECT COUNT(*) FROM lottery_draws WHERE week_start = '$weekStart'");
                $cnt = (int)$countStmt->fetchColumn();
                // Shift week_start by cnt days to make it unique
                $adjustedStart = (clone $now)->modify('-' . ($dow - 1) . ' days')->modify('+' . $cnt . ' seconds')->format('Y-m-d');
                // Actually, let's just use today as custom start
                $adjustedStart = $now->format('Y-m-d');
                $adjustedEnd = $weekEnd;
                // Check if this exact start already exists
                $chk = $pdo->prepare("SELECT id FROM lottery_draws WHERE week_start = ? AND status = 'active'");
                $chk->execute([$adjustedStart]);
                if ($chk->fetchColumn()) {
                    echo json_encode(['success' => false, 'error' => 'An active draw already exists for today.']);
                    exit;
                }
                // Try insert, ignore duplicate
                try {
                    $pdo->prepare("INSERT INTO lottery_draws (week_start, week_end, status) VALUES (?, ?, 'active')")->execute([$adjustedStart, $adjustedEnd]);
                } catch (Exception $e2) {
                    // Duplicate key — just reactivate
                    $pdo->prepare("UPDATE lottery_draws SET status = 'active', winner_user_id = NULL, winner_ticket_id = NULL, drawn_at = NULL WHERE week_start = ?")->execute([$adjustedStart]);
                }
                $newId = $pdo->lastInsertId() ?: $pdo->query("SELECT id FROM lottery_draws WHERE week_start = '$adjustedStart' AND status = 'active' ORDER BY id DESC LIMIT 1")->fetchColumn();
                echo json_encode(['success' => true, 'message' => 'New draw created! ('. $adjustedStart .' — '. $adjustedEnd .')', 'draw_id' => $newId]);
            } else {
                $pdo->prepare("INSERT INTO lottery_draws (week_start, week_end, status) VALUES (?, ?, 'active')")->execute([$weekStart, $weekEnd]);
                $newId = $pdo->lastInsertId();
                echo json_encode(['success' => true, 'message' => 'New draw created! ('. $weekStart .' — '. $weekEnd .')', 'draw_id' => $newId]);
            }
        } catch (Exception $e) {
            echo json_encode(['success' => false, 'error' => $e->getMessage()]);
        }
...
$_lottery_req_ads = getSetting('lottery_req_ads');
$lottery_req_ads = ($_lottery_req_ads !== null && $_lottery_req_ads !== '') ? $_lottery_req_ads : '10';
$_lottery_req_earn = getSetting('lottery_req_earn');
$lottery_req_earn = ($_lottery_req_earn !== null && $_lottery_req_earn !== '') ? $_lottery_req_earn : '15';
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                <div>
                                    ...
                                    <div class="text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-3 mt-1">
                                        ...
                                        <span><i class="fas fa-coins text-violet-500 mr-1"></i> Pool: <?php echo number_format((float)($d['prize_pool'] ?? 0), 2); ?> | Total: <?php echo number_format((float)(getSetting('lottery_prize') ?: 0) + (float)($d['prize_pool'] ?? 0), 2); ?></span>
